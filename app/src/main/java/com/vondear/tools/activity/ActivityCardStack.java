...
public class ActivityCardStack extends ActivityBase implements RxCardStackView.ItemExpendListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mStackView.setNumBottomShow(4);

        new Handler().postDelayed(
                new Runnable() {
                    @Override
                    public void run() {
                        mTestStackAdapter.updateData(Arrays.asList(TEST_DATAS),true);
                    }
                }
                , 200
        );
    ...
    public void onPreClick(View view) {
        RxToast.warning(mStackView.getRxScrollDelegate().getViewScrollY() + "");
        scrollY=mStackView.getRxScrollDelegate().getViewScrollY();
    }

    public void onNextClick(View view) {
        RxToast.warning(mStackView.getRxScrollDelegate().getViewScrollY() + "");
        mStackView.getRxScrollDelegate().scrollViewTo(0, scrollY);
    }
