...
import com.vondear.rxtools.constants.Q;
...
public class ActivityScanerCode extends ActivityBase implements SurfaceHolder.Callback {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (bundle != null && bundle.containsKey(Q.action.isFlag)) {
            isFlag = bundle.getBoolean(Q.action.isFlag);
        }
    ...
    private void initCamera(SurfaceHolder surfaceHolder) {
        ...
        if (handler == null) {
            handler = new CaptureActivityHandler(new WeakReference<ActivityScanerCode>(ActivityScanerCode.this));
        }
    }
...
//                        RxUtils.showToast(ActivityScanerCode.this, "" + phone, false);
                        ...
                        intent.putExtra(Q.param.ScanResult, phone);
...
//        String type = phone.substring(0, 4);
        final String realContent = phone;
        /*if ("二维码:".equals(type)) {
        ...
        rxDialogSure.getTvTitle().setText("扫描结果：");
        rxDialogSure.setLogo(android.R.drawable.ic_dialog_alert);
        rxDialogSure.setContent(realContent);
        rxDialogSure.getTvSure().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                String spCheck = RxSPUtils.getContent(context, Q.param.AssetDataCount);
                spCheck += realContent + ";";
                RxSPUtils.putContent(context, Q.param.AssetDataCount, spCheck);
                rxDialogSure.cancel();
            }
        });
...
//        RxSPUtils.putContent(context, SP_SCAN_CODE, RxDataUtils.stringToInt(RxSPUtils.getContent(context, SP_SCAN_CODE)) + 1 + "");
        if (isFlag) {
            if (StringUtils.isEmpty(result) | result.indexOf("\n") == -1) return;
            result = result.substring(0, result.indexOf("\n"));
            dialogShow(result);
        } else {
            Intent intent = new Intent();
            intent.putExtra(Q.param.ScanResult, result);
            setResult(RESULT_OK, intent);
            finish();
        }
